CAC_VIEW = {
  ...
  renderThirdPartyCalGroups: function (
    ...
    this.THIRD_PARTY_CALENDARS.forEach((target) => {
      layerGroups.append(`<a href="javascript:void(0)" title="${target.title}" onclick="CAC_VIEW.handleThirdPartyCalClick('${target.name}', '${calendarGroupId}', '${calendarId}', '${title}', '${start_datetime}', '${end_datetime}', '${description}', '${allDay}')">
        <img src="/calendar/app/img/ico_calendar_${target.name}.png" style="${style}" alt="${target.title}" /></a>`);
    });
  },
