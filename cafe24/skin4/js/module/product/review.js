            ...
            success: function(data) {
                ...
                if (data.is_secret == true) {
                    ...
                    if (data.comment != undefined) {
                        aHtml.push('<div class="gLeft"><a href="#none" class="" onclick="REVIEW.comment_view('+data.read['no']+');">댓글보기 <em>'+data.read['comment_count']+'</em></a></div>');
                        aHtml.push('<div class="gRight">');
                        if (data.write_auth == true) {
                            aHtml.push('<a href="/board/product/modify.html?board_act=edit&no='+data.no+'&board_no=4&link_product_no='+iProductNo+'" class="">수정</a>');
                        }
                        aHtml.push('<a href="#none" class="" onclick="REVIEW.comment_write(this);">쓰기</a>');
                        aHtml.push('</div>');
                    }
                    aHtml.push('</div>');
                    aHtml.push('</div>');
