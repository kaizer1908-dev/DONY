...
var QNA = {
            ...
            success: function(data) {
                ...
                if (data.is_secret == true) {
                    ...
                    if (data.comment != undefined) {
                        aHtml.push('<div class="gLeft"><a href="#none" class="" onclick="QNA.comment_view('+data.read['no']+');">댓글<em>'+data.read['comment_count']+'</em></a></div>');
                        aHtml.push('<div class="gRight"><a href="/board/product/modify.html'+ data.read['param_modify'] +'&link_product_no='+iProductNo+'" class="">수정</a><a href="#none" class="" onclick="QNA.comment_write(this);">쓰기</a></div>');
                    }
                    aHtml.push('</div>');
                    aHtml.push('</div>');
                    ...
                    if (data.comment_write != undefined) {
                        ...
                        aHtml.push('<div class="captcha ' +data.comment_write['use_captcha']+ '"><span class="img"></span><div class="form"><div class="flex">' +data.comment_write['captcha_image']+data.comment_write['captcha_refresh']+'</div>'+data.comment_write['captcha']+ '<p class="ec-base-help">왼쪽의 문자를 공백없이 입력하세요.(대소문자구분)</p></div></div>');
                        aHtml.push('</div>');
                        aHtml.push('</form>');
                    }
                    ...
                    if (data.comment_reply != undefined) {
                        ...
                        aHtml.push('<div class="captcha ' +data.comment_reply['use_captcha']+ '"><span class="img"></span><div class="form"><div class="flex">' +data.comment_reply['captcha_image']+data.comment_reply['captcha_refresh']+'</div>'+data.comment_reply['captcha']+ '<p class="ec-base-help">왼쪽의 문자를 공백없이 입력하세요.(대소문자구분)</p></div></div>');
                        aHtml.push('</div>');
                        aHtml.push('</form>');
                    }
                }
    ...
    comment_write : function (e)
    {
        ...
        if ($form.css('display') == 'none') {
            ...
            $('.xans-product-additional .writeForm .infoForm input[type=text]').attr('placeholder', '작성자')
            $('.xans-product-additional .writeForm .infoForm input[type=password]').attr('placeholder', '비밀번호')
